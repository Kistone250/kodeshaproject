...
$amount = $_POST['amount'];
$date = $_POST['date'];
if(empty($fname) OR empty($lname) OR empty($phone) OR empty($email) OR empty($location) OR empty($amount)){
...
if(!filter_var($email, FILTER_VALIDATE_EMAIL)){
	echo "<script> alert (' Email not valid')</script>";
		...
		$query = "INSERT INTO request VALUES ('','$fname','$lname','$phone','$email','$location','$amount','$date')";
		mysqli_query($conn,$query);
		$insertdate = date('Y-m-d', strtotime($_POST['date']));
		echo "<script> alert ('Property Requested successfully click ok to proceed')</script>";
	}
}
?>
<!DOCTYPE html>
...
<div id="form">
<form action="req.php" method="POST" autocomplete="off">
	...
	<p>Requesting date:</p><input type="date" name="date" >
	<p><input type="checkbox" name="agree"> By requesting you agree to our terms and conditions???</p>
